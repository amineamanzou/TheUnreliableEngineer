...
La discussion arrive souvent par une démonstration très convaincante. Un agent écrit une fonction, corrige un test et prépare une pull request pendant que nous regardons l’écran. À ce moment-là, la conclusion semble presque déjà écrite : si la machine fait cela en quelques minutes, combien de développeurs faudra-t-il encore demain ?

Je comprends le raccourci. La démonstration est réelle, le gain peut l’être aussi. Seulement, elle nous montre la partie du travail qui tient bien dans une fenêtre. Elle ne nous montre ni la revue, ni l’intégration, ni la décision métier, ni le téléphone qui sonnera si le changement se comporte autrement en production.

En juillet 2025, METR publie justement une expérience qui contrarie le récit du moment. Seize développeurs expérimentés travaillent sur 246 tâches dans des projets open source qu’ils connaissent depuis plusieurs années. Les tâches sont réparties aléatoirement entre deux conditions : outils d’IA autorisés ou interdits. Résultat mesuré sur ce périmètre : avec l’IA disponible début 2025, les développeurs prennent environ 19 % de temps en plus.
...
Vous allez peut-être me dire que, dans votre équipe, l’IA vous fait déjà gagner du temps. C’est tout à fait possible. L’étude ne dit pas que votre expérience est fausse. Elle rappelle qu’un gain dépend de la tâche, du contexte, de l’outil et surtout de l’endroit où nous décidons d’arrêter le chronomètre.
...
Prenez votre dernier incident ou votre dernière mise en production un peu sensible. Combien de temps avez-vous réellement passé à taper du code ? Et combien à retrouver l’historique, appeler la bonne personne, comprendre une convention jamais documentée ou arbitrer entre deux mauvaises options ? C’est souvent dans ce second inventaire que le mot « remplacer » perd sa netteté.
...
Cela ne retire rien aux gains que certaines équipes voient déjà. Vous pouvez livrer plus vite sur un périmètre bien maîtrisé et, en même temps, découvrir que la revue ou la stabilité devient votre nouvelle limite. Les deux observations peuvent être vraies. C’est même souvent le signe que l’outil a déplacé le problème au lieu de le faire disparaître.
...
Et je ne dis pas cela pour transformer chaque suggestion en procédure de contrôle interminable. Dans beaucoup d’équipes, la revue est déjà le point de tension. Lui envoyer davantage de changements sans regarder son coût revient simplement à rendre invisible le travail des personnes qui absorbent le risque.
...
Vous n’avez pas besoin de lancer un programme de mesure sur toute l’entreprise pour commencer. Prenez un flux de livraison connu, pendant une période limitée, et comparez des cohortes ou des périodes. Enregistrez au minimum : temps actif et temps d’attente, taille des changements, taux de reprise, défauts découverts après revue, incidents, temps de revue senior et résultat pour l’utilisateur. Gardez aussi une trace des tâches confiées à l’IA : explication, génération, migration, test, investigation ou décision.
...
La mesure sera imparfaite. Certains changements seront plus difficiles que d’autres et les outils évolueront pendant l’expérience. Ce n’est pas une raison pour abandonner. Une frontière de mesure explicite et quelques exemples relus ensemble valent déjà mieux qu’un taux d’adoption présenté comme une preuve de productivité.
...
Si votre organisation n’est pas prête à mesurer toute cette chaîne, commencez par une question plus modeste : où le temps gagné à la génération réapparaît-il ensuite ? Dans la revue, les tests, la coordination ou l’exploitation ? Cette conversation suffit parfois à choisir une première amélioration sans prétendre prévoir le métier à cinq ans.
